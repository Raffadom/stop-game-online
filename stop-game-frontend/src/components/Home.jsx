...
export default function Home({ onJoinOrCreateRoom, roomError, isConnected }) {
    ...
    useEffect(() => {
        let storedUserId = localStorage.getItem('userId'); // Usar 'userId' como chave
        if (!storedUserId) {
            storedUserId = uuidv4();
    ...
    const handleJoin = () => {
    console.log("handleJoin foi chamado! (Teste de clique)"); // <-- ADICIONE ESTA LINHA
    ...
    return (
        <div className="max-w-md mx-auto bg-white p-8 rounded-2xl shadow-xl space-y-6 border border-gray-200">
            ...
            <div className="space-y-4">
                ...
                {(localError || roomError) && (
                    <p className="text-red-500 text-sm text-center">{localError || roomError}</p>
                )}
            </div>

            <button
              onClick={handleJoin} // <-- VERIFIQUE SE ESTÁ EXATAMENTE ASSIM
                className="w-full bg-blue-600 hover:bg-blue-700 text-white p-3 rounded-lg font-semibold text-xl shadow-md transition-colors duration-200"
